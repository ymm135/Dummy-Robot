...
/* USER CODE BEGIN 0 */
// 机械臂相关说明：
// - I2C1 建议连接 IMU/姿态传感器；I2C2 可接 OLED/屏；I2C3 预留其它传感器。
// - 时钟 400kHz：满足姿态/温度/触觉等中低速采样；控制周期内建议批量读取以降低抖动。
// - 访问策略：传感采集置于低优先级任务，使用互斥锁保护总线；将原始数据放入环形缓冲，核心控制周期再做融合。
// - 错误处理：若出现 NACK/超时，回退到安全姿态或忽略该周期传感，避免阻塞运动控制。
/* USER CODE END 0 */
